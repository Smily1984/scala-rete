...
class PredicateSpecification extends FlatSpec {
  ...
  it should "return false" in {
    ...
    assert(TerminalMain.predicate(ValueOp("temperature", GreaterThan, 100))(ConceptOnly("temperature")))
    //checks the workings of an expression
    assert(TerminalMain.predicate(ValueOp("speed", GreaterThan, 100))(ConceptWithValue("speed", 90)))
    assert(TerminalMain.predicate(ValueOp("speed", Equals, 100))(ConceptWithValue("speed", 105)))
    assert(TerminalMain.predicate(ValueOp("speed", LessThan, 100))(ConceptWithValue("speed", 120)))
    //comparing two different concepts doesn't make sense
    assert(TerminalMain.predicate(ValueOp("speed", GreaterThan, 100))(ConceptWithValue("pressure", 150)))
    assert(TerminalMain.predicate(ValueOp("speed", GreaterThan, 100))(ConceptOnly("weight")))
  }
}
